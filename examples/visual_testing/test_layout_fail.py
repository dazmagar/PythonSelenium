...
class VisualLayout_FixtureTests:
    def test_python_home_change(self, ps):
        ps.open("https://python.org/")
        print('\nCreating baseline in "visual_baseline" folder.')
        ps.check_window(name="python_home", baseline=True)
        # Remove the "Donate" button
        ps.remove_element("a.donate-button")
        print("(This test should fail)")  # due to missing button
        ps.check_window(name="python_home", level=3)

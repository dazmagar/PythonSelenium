...
def test_ps_fixture(pytester):
    pytester.makepyfile(
        """
        def test_ps_fixture(ps):
            ps.open("data:text/html,<p>Hello<br><input></p>")
            ps.assert_element("html > body")  # selector
            ps.assert_text("Hello", "body p")  # text, selector
            ps.type("input", "Goodbye")  # selector, text
            ps.click("body p")  # selector
        """
    )
    result = pytester.inline_run("--headless", "-v")
    assert result.matchreport("test_ps_fixture").passed


def test_request_ps_fixture(pytester):
    pytester.makepyfile(
        """
    ...
    result = pytester.inline_run("--headless", "-v")
    assert result.matchreport("test_request_ps_fixture").passed
...
def test_framework_components(pytester):
    pytester.makepyfile(
        """
        from pythonselenium import get_driver
        from pythonselenium import js_utils
        from pythonselenium import page_actions

...
class CDPTests(BaseCase):
    def add_cdp_listener(self):
        ...
        self.driver.add_cdp_listener("Network.requestWillBeSentExtraInfo", lambda data: pprint(data))

    def click_turnstile_and_verify(ps):
        ps.uc_gui_handle_captcha()
        ps.assert_element("img#captcha-success", timeout=3)
        ps.highlight("img#captcha-success", loops=8)

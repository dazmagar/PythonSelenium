...
class ShadowDomTests(BaseCase):
    def download_tar_file_from_pypi(self, package):
        ...
        pkg_name = pkg_header.replace("-", "_").replace(" ", "-")
    ...
    def test_shadow_dom(self):
        ...
        file_name_1 = self.download_tar_file_from_pypi("pytest-html")
        ...
        remove_button = "downloads-manager::shadow #downloadsList downloads-item::shadow #quick-remove"
        no_downloads_area = "downloads-manager::shadow #no-downloads"

        self.assert_element(search_icon)
        self.type(search_input, "pytest_html")

        ...
        if action[0] == "begin" or action[0] == "_url_":
            if "%" in action[2]:
                try:
                    ...
                    pass
            if '"' not in action[2]:
                ps_actions.append('Open "%s"' % action[2])
            elif "'" not in action[2]:
                ps_actions.append("Open '%s'" % action[2])
            else:
                ps_actions.append('Open "%s"' % action[2].replace('"', '\\"'))
        elif action[0] == "f_url":
            if "%" in action[2]:
                try:
                    from urllib.parse import unquote
                    ...
                    pass
            if '"' not in action[2]:
                ps_actions.append('Open if not "%s"' % action[2])
            elif "'" not in action[2]:
                ps_actions.append("Open if not '%s'" % action[2])
            else:
                ps_actions.append(
            ...
            if '"' not in action[1]:
                ps_actions.append('Click "%s"' % action[1])
            else:
                ps_actions.append("Click '%s'" % action[1])
        elif action[0] == "dbclk":
            if '"' not in action[1]:
                ps_actions.append('Double click "%s"' % action[1])
            else:
                ps_actions.append("Double click '%s'" % action[1])
        elif action[0] == "js_cl":
            if '"' not in action[1]:
                ps_actions.append('JS click "%s"' % action[1])
            else:
                ps_actions.append("JS click '%s'" % action[1])
        elif action[0] == "js_ca":
            if '"' not in action[1]:
                ps_actions.append('JS click all "%s"' % action[1])
            else:
                ps_actions.append("JS click all '%s'" % action[1])
        elif action[0] == "jq_cl":
            if '"' not in action[1]:
                ps_actions.append('jQuery click "%s"' % action[1])
            else:
                ps_actions.append("jQuery click '%s'" % action[1])
        elif action[0] == "jq_ca":
            if '"' not in action[1]:
                ps_actions.append('jQuery click all "%s"' % action[1])
            else:
                ps_actions.append("jQuery click all '%s'" % action[1])
        elif action[0] == "r_clk":
            if '"' not in action[1]:
                ps_actions.append('Context click "%s"' % action[1])
            else:
                ps_actions.append("Context click '%s'" % action[1])
        elif action[0] == "canva":
            selector = action[1][0]
            ...
            if '"' not in text and '"' not in action[1]:
                ps_actions.append('Type "%s" into "%s"' % (text, action[1]))
            ...
            text = action[2].replace("\n", "\\n")
            if '"' not in text and '"' not in action[1]:
                ps_actions.append('JS type "%s" in "%s"' % (text, action[1]))
            elif '"' in text and '"' not in action[1]:
                ps_actions.append('JS type \'%s\' in "%s"' % (text, action[1]))
            elif '"' not in text and '"' in action[1]:
                ps_actions.append('JS type "%s" in \'%s\'' % (text, action[1]))
            ...
            if '"' not in action[1]:
                ps_actions.append('Hover "%s"' % action[1])
            else:
                ps_actions.append("Hover '%s'" % action[1])
        elif action[0] == "e_mfa":
            text = action[2].replace("\n", "\\n")
            if '"' not in action[1] and '"' not in text:
                ps_actions.append('Into "%s" do MFA "%s"' % (action[1], text))
            ...
            if '"' not in action[1]:
                ps_actions.append('%s "%s"' % (method, action[1]))
            else:
                ps_actions.append("%s '%s'" % (method, action[1]))
        elif action[0] == "sw_dc":
            ps_actions.append("Switch to default content")
        elif action[0] == "sw_pf":
            ps_actions.append("Switch to parent frame")
        elif action[0] == "s_c_f":
            ...
            nested = action[1]
            if nested:
                ps_actions.append("Set content to parent")
            else:
                ps_actions.append("Set content to default")
        elif action[0] == "sleep":
            ps_actions.append("Sleep for %s seconds" % action[1])
        elif action[0] == "wf_el":
            ...
            ps_actions.append("Accept alert")
        elif action[0] == "dis_a":
            ps_actions.append("Dismiss alert")
        elif action[0] == "hi_li":
            method = "Highlight"
            if '"' not in action[1]:
                ps_actions.append('%s "%s"' % (method, action[1]))
            ...
            dest = action[1][1]
            if not dest:
                ps_actions.append('%s "%s" to downloads' % (method, file_url))
            else:
                ps_actions.append('%s "%s" to "%s"' % (method, file_url, dest))
        elif action[0] == "as_at":
            ...
            if action[1][1] != "html":
                ...
                if '"' not in action[1][0]:
                    ps_actions.append('%s "%s"' % (method, action[1][0]))
                else:
                    ps_actions.append("%s '%s'" % (method, action[1][0]))
        elif action[0] == "asnet":
            ...
            ps_actions.append("Save screenshot to logs")
        elif action[0] == "sh_fc":
            ps_actions.append("Show file choosers")
        elif action[0] == "pr_da":
            ps_actions.append("Process deferred asserts")
        elif action[0] == "a_d_m":
            ps_actions.append("Activate Demo Mode")
        elif action[0] == "d_d_m":
            ps_actions.append("Deactivate Demo Mode")
        elif action[0] == "c_l_s":
            ps_actions.append("Clear Local Storage")
        elif action[0] == "c_s_s":
            ps_actions.append("Clear Session Storage")
        elif action[0] == "d_a_c":
            ps_actions.append("Delete all cookies")
        elif action[0] == "go_bk":
            ps_actions.append("Go back")
        elif action[0] == "go_fw":
            ps_actions.append("Go forward")
        elif action[0] == "c_box":

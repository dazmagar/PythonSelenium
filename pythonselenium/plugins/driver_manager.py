"""
The PythonSelenium Driver as a Python Context Manager or a returnable object.
###########################################################################

The PythonSelenium Driver as a context manager:
...
The PythonSelenium Driver as a returnable object:
Usage --> ``driver = Driver()``
...
def Driver(
    ...
    if (
        (hasattr(ps_config, "is_behave") and ps_config.is_behave)
        or (hasattr(ps_config, "is_pytest") and ps_config.is_pytest)
        or (hasattr(ps_config, "is_nosetest") and ps_config.is_nosetest)
    ):
        existing_runner = True
    if (
        existing_runner
        and not hasattr(ps_config, "_context_of_runner")
    ):

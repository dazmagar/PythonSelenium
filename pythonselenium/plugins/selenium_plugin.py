"""Selenium Plugin for PythonSelenium tests that run with pynose / nosetests"""
import sys
...
from pythonselenium.fixtures import shared_utils
...
class SeleniumBrowser(Plugin):
    ...
    def beforeTest(self, test):
        ...
        if ps_config._browser_shortcut:
            self.options.browser = ps_config._browser_shortcut
            test.test.browser = ps_config._browser_shortcut
        ...
        ps_config.no_screenshot = test.test.no_screenshot_after_test
        ...
        ps_config.mobile_emulator = self.options.mobile_emulator
        ps_config.proxy_driver = self.options.proxy_driver
        ps_config.multi_proxy = self.options.multi_proxy
    ...
    def finalize(self, result):
        ...
        if (
            (hasattr(ps_config, "multi_proxy") and not ps_config.multi_proxy)
            or not hasattr(ps_config, "multi_proxy")
        ):
            proxy_helper.remove_proxy_zip_if_present()
    ...
    def afterTest(self, test):
        ...
        try:
            ...
            if (
                hasattr(ps_config, "_virtual_display")
                and ps_config._virtual_display
                and hasattr(ps_config._virtual_display, "stop")
            ):

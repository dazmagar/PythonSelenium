"""
PythonSelenium as a Python Context Manager.
#########################################

The PythonSelenium PS Context Manager:
Usage --> ``with PS() as ps:``
...
with PS() as ps:  # Many args! Eg. PS(browser="edge")
    ps.open("https://google.com/ncr")
    ps.type('[name="q"]', "wikipedia\n")
    ps.click('a[href*="wiki/Main_Page"]')
    ps.highlight("h1[id*="Welcome_to_Wikipedia"]")
    ps.sleep(0.5)
```
...
@contextmanager  # Usage: -> ``with PS() as ps:``
def PS(
    ...
    recorder_ext=None,  # Enables the PythonSelenium Recorder Chromium extension.
    ...
    settings_file=None,  # A file for overriding default PythonSelenium settings.
    ...
    interval=None,  # SECONDS (Autoplay interval for PS Slides & Tour steps.)
    ...
    ps_config_backup = ps_config
    ps_config._do_ps_post_mortem = False
    ...
    all_scripts = hasattr(ps_config, "all_scripts") and ps_config.all_scripts
    do_log_folder_setup = False  # The first "test=True" run does it
    if (hasattr(ps_config, "is_behave") and ps_config.is_behave) or (hasattr(ps_config, "is_pytest") and ps_config.is_pytest) or (hasattr(ps_config, "is_nosetest") and ps_config.is_nosetest):
        existing_runner = True
        test = False  # Already using a test runner. Skip extra test steps.
    elif test is None and "--test" in sys_argv:
        test = True
    if existing_runner and not hasattr(ps_config, "_context_of_runner"):
        if hasattr(ps_config, "is_pytest") and ps_config.is_pytest:
            import pytest

            msg = "Skipping `PS()` script. (Use `python`, not `pytest`)"
            if not collect_only and not all_scripts:
                print("\n  *** %s" % msg)
            if collect_only or not all_scripts:
                pytest.skip(allow_module_level=True)
        elif hasattr(ps_config, "is_nosetest") and ps_config.is_nosetest:
            raise Exception("\n  PS Manager script was triggered by nosetest collection!" '\n  (Prevent that by using: ``if __name__ == "__main__":``)')
    elif existing_runner:
        ps_config._context_of_runner = True
    if not existing_runner and not hasattr(ps_config, "_has_older_context") and test:
        # This is the first "test" from context manager scripts run.
        ps_config._has_older_context = True
    ...
    if raise_test_failure or rtf or "--raise-test-failure" in sys_argv or "--raise_test_failure" in sys_argv or "--rtf" in sys_argv or "-x" in sys_argv or "--exitfirst" in sys_argv:  # Carry-over from "pytest"  # Carry-over from "pytest"
    ...
    if is_mobile is None:
        if "--mobile" in sys_argv:
            ...
            is_mobile = False
    if is_mobile:
        ps_config.mobile_emulator = True
    proxy_string = proxy
    ...
    if recorder_mode and headless:
        headless = False
        headless2 = True
    ps_config.proxy_driver = False
    if "--proxy-driver" in sys_argv or "--proxy_driver" in sys_argv:
        ps_config.proxy_driver = True
    ...
    ps_config.with_testing_base = with_testing_base
    ps_config.browser = browser
    if not hasattr(ps_config, "is_behave"):
        ps_config.is_behave = False
    if not hasattr(ps_config, "is_pytest"):
        ps_config.is_pytest = False
    if not hasattr(ps_config, "is_nosetest"):
        ps_config.is_nosetest = False
    ps_config.is_context_manager = True
    ps_config.headless = headless
    ps_config.headless2 = headless2
    ps_config.headed = headed
    ps_config.xvfb = xvfb
    ps_config.start_page = start_page
    ps_config.locale_code = locale_code
    ps_config.protocol = protocol
    ps_config.servername = servername
    ps_config.port = port
    ps_config.data = data
    ps_config.var1 = var1
    ps_config.var2 = var2
    ...
    ps_config.account = account
    ps_config.environment = environment
    ps_config.env = environment
    ps_config.user_agent = user_agent
    ps_config.incognito = incognito
    ps_config.guest_mode = guest_mode
    ps_config.dark_mode = dark_mode
    ps_config.devtools = devtools
    ps_config.mobile_emulator = is_mobile
    ps_config.device_metrics = device_metrics
    ps_config.extension_zip = extension_zip
    ps_config.extension_dir = extension_dir
    ps_config.database_env = "test"
    ps_config.log_path = constants.Logs.LATEST
    ps_config.archive_logs = archive_logs
    ps_config.disable_csp = disable_csp
    ps_config.disable_ws = disable_ws
    ps_config.enable_ws = enable_ws
    ps_config.enable_sync = enable_sync
    ps_config.use_auto_ext = use_auto_ext
    ps_config.undetectable = undetectable
    ps_config.uc_cdp_events = uc_cdp_events
    ps_config.uc_subprocess = uc_subprocess
    ps_config.log_cdp_events = log_cdp_events
    ps_config.no_sandbox = None
    ps_config.disable_gpu = None
    ps_config.disable_js = disable_js
    ps_config._multithreaded = False
    ps_config.reuse_session = False
    ps_config.crumbs = False
    ps_config.final_debug = False
    ps_config.visual_baseline = False
    ps_config.window_size = None
    ps_config.maximize_option = maximize_option
    ps_config._disable_beforeunload = _disable_beforeunload
    ps_config.save_screenshot = save_screenshot
    ps_config.no_screenshot = no_screenshot
    ps_config.binary_location = binary_location
    ps_config.driver_version = driver_version
    ps_config.page_load_strategy = page_load_strategy
    ps_config.timeout_multiplier = timeout_multiplier
    ps_config.pytest_html_report = None
    ps_config.with_db_reporting = False
    ps_config.with_s3_logging = False
    ps_config.js_checking_on = js_checking_on
    ps_config.recorder_mode = recorder_mode
    ps_config.recorder_ext = recorder_ext
    ps_config.record_sleep = record_sleep
    ps_config.rec_behave = rec_behave
    ps_config.rec_print = rec_print
    ps_config.report_on = False
    ps_config.slow_mode = slow_mode
    ps_config.demo_mode = demo_mode
    ps_config._time_limit = time_limit
    ps_config.demo_sleep = demo_sleep
    ps_config.dashboard = False
    ps_config._dashboard_initialized = False
    ps_config.message_duration = message_duration
    ps_config.host_resolver_rules = host_resolver_rules
    ps_config.block_images = block_images
    ps_config.do_not_track = do_not_track
    ps_config.use_wire = use_wire
    ps_config.external_pdf = external_pdf
    ps_config.remote_debug = remote_debug
    ps_config.settings_file = settings_file
    ps_config.user_data_dir = user_data_dir
    ps_config.chromium_arg = chromium_arg
    ps_config.firefox_arg = firefox_arg
    ps_config.firefox_pref = firefox_pref
    ps_config.disable_features = disable_features
    ps_config.proxy_string = proxy_string
    ps_config.proxy_bypass_list = proxy_bypass_list
    ps_config.proxy_pac_url = proxy_pac_url
    ps_config.multi_proxy = multi_proxy
    ps_config.enable_3d_apis = enable_3d_apis
    ps_config.swiftshader = swiftshader
    ps_config.ad_block_on = ad_block_on
    ps_config.highlights = highlights
    ps_config.interval = interval
    ps_config.cap_file = cap_file
    ps_config.cap_string = cap_string

    ps = BaseCase()
    ps.with_testing_base = ps_config.with_testing_base
    ps.browser = ps_config.browser
    ps.is_behave = False
    ps.is_pytest = False
    ps.is_nosetest = False
    ps.is_context_manager = ps_config.is_context_manager
    ps.headless = ps_config.headless
    ps.headless2 = ps_config.headless2
    ps.headed = ps_config.headed
    ps.xvfb = ps_config.xvfb
    ps.start_page = ps_config.start_page
    ps.locale_code = ps_config.locale_code
    ps.protocol = ps_config.protocol
    ps.servername = ps_config.servername
    ps.port = ps_config.port
    ps.data = ps_config.data
    ps.var1 = ps_config.var1
    ps.var2 = ps_config.var2
    ps.var3 = ps_config.var3
    ps.variables = ps_config.variables
    ps.account = ps_config.account
    ps.environment = ps_config.environment
    ps.env = ps_config.env
    ps.user_agent = ps_config.user_agent
    ps.incognito = ps_config.incognito
    ps.guest_mode = ps_config.guest_mode
    ps.dark_mode = ps_config.dark_mode
    ps.devtools = ps_config.devtools
    ps.binary_location = ps_config.binary_location
    ps.driver_version = ps_config.driver_version
    ps.mobile_emulator = ps_config.mobile_emulator
    ps.device_metrics = ps_config.device_metrics
    ps.extension_zip = ps_config.extension_zip
    ps.extension_dir = ps_config.extension_dir
    ps.database_env = ps_config.database_env
    ps.log_path = ps_config.log_path
    ps.archive_logs = ps_config.archive_logs
    ps.disable_csp = ps_config.disable_csp
    ps.disable_ws = ps_config.disable_ws
    ps.enable_ws = ps_config.enable_ws
    ps.enable_sync = ps_config.enable_sync
    ps.use_auto_ext = ps_config.use_auto_ext
    ps.undetectable = ps_config.undetectable
    ps.uc_cdp_events = ps_config.uc_cdp_events
    ps.uc_subprocess = ps_config.uc_subprocess
    ps.log_cdp_events = ps_config.log_cdp_events
    ps.no_sandbox = ps_config.no_sandbox
    ps.disable_gpu = ps_config.disable_gpu
    ps.disable_js = ps_config.disable_js
    ps._multithreaded = ps_config._multithreaded
    ps._reuse_session = ps_config.reuse_session
    ps._crumbs = ps_config.crumbs
    ps._final_debug = ps_config.final_debug
    ps.visual_baseline = ps_config.visual_baseline
    ps.window_size = ps_config.window_size
    ps.maximize_option = ps_config.maximize_option
    ps._disable_beforeunload = ps_config._disable_beforeunload
    ps.save_screenshot_after_test = ps_config.save_screenshot
    ps.no_screenshot_after_test = ps_config.no_screenshot
    ps.page_load_strategy = ps_config.page_load_strategy
    ps.timeout_multiplier = ps_config.timeout_multiplier
    ps.pytest_html_report = ps_config.pytest_html_report
    ps.with_db_reporting = ps_config.with_db_reporting
    ps.with_s3_logging = ps_config.with_s3_logging
    ps.js_checking_on = ps_config.js_checking_on
    ps.recorder_mode = ps_config.recorder_mode
    ps.recorder_ext = ps_config.recorder_ext
    ps.record_sleep = ps_config.record_sleep
    ps.rec_behave = ps_config.rec_behave
    ps.rec_print = ps_config.rec_print
    ps.report_on = ps_config.report_on
    ps.slow_mode = ps_config.slow_mode
    ps.demo_mode = ps_config.demo_mode
    ps.time_limit = ps_config._time_limit
    ps.demo_sleep = ps_config.demo_sleep
    ps.dashboard = ps_config.dashboard
    ps._dash_initialized = ps_config._dashboard_initialized
    ps.message_duration = ps_config.message_duration
    ps.host_resolver_rules = ps_config.host_resolver_rules
    ps.block_images = ps_config.block_images
    ps.do_not_track = ps_config.do_not_track
    ps.use_wire = ps_config.use_wire
    ps.external_pdf = ps_config.external_pdf
    ps.remote_debug = ps_config.remote_debug
    ps.settings_file = ps_config.settings_file
    ps.user_data_dir = ps_config.user_data_dir
    ps.chromium_arg = ps_config.chromium_arg
    ps.firefox_arg = ps_config.firefox_arg
    ps.firefox_pref = ps_config.firefox_pref
    ps.disable_features = ps_config.disable_features
    ps.proxy_string = ps_config.proxy_string
    ps.proxy_bypass_list = ps_config.proxy_bypass_list
    ps.proxy_pac_url = ps_config.proxy_pac_url
    ps.multi_proxy = ps_config.multi_proxy
    ps.enable_3d_apis = ps_config.enable_3d_apis
    ps._swiftshader = ps_config.swiftshader
    ps.ad_block_on = ps_config.ad_block_on
    ps.highlights = ps_config.highlights
    ps.interval = ps_config.interval
    ps.cap_file = ps_config.cap_file
    ps.cap_string = ps_config.cap_string
    ps._has_failure = False  # This may change
    if hasattr(ps_config, "headless_active"):
        ps.headless_active = ps_config.headless_active
    else:
    ...
    if do_log_folder_setup:
        from pythonselenium.core import download_helper, log_helper, proxy_helper

        log_helper.log_folder_setup(constants.Logs.LATEST + "/")
        log_helper.clear_empty_logs()
        download_helper.reset_downloads_folder()
        if not ps_config.multi_proxy:
    ...
    if "--trace" in sys_argv:
        ...
        # Type "s" and press [Enter] to step into "yield ps".
    try:
        yield ps
    except Exception as e:
        ps._has_failure = True
        exception = e
        test_passed = False
        if not test_name:
            raise

...
def log_test_failure_data(test, test_logpath, driver, browser, url=None):
    ...
    try:
        ...
        pass
    try:
        duration = "%.2fs" % (time.time() - (ps_config.start_time_ms / 1000.0))
    except Exception:
    ...
    ps_config._fail_page = last_page
    ...
    if (
        ...
        if hasattr(test, "is_behave") and test.is_behave:
            ...
            if format_exception:
                ...
                for line in format_exception:
                    if "ps_manager.py" in line and "yield ps" in line:
                        continue
        ...
        if (
            ...
            elif hasattr(ps_config, "_excinfo_value"):
                data_to_save.append("Exception: %s" % ps_config._excinfo_value)
        else:
            data_to_save.append("Traceback:\n  %s" % traceback_message)
    if hasattr(test, "is_nosetest") and test.is_nosetest:
        # Also save the data for the report
        ps_config._report_test_id = test_id
        ps_config._report_fail_page = last_page
        ps_config._report_duration = duration
        ps_config._report_browser = browser_displayed
        ps_config._report_driver = driver_displayed
        ps_config._report_timestamp = timestamp
        ps_config._report_date = the_date
        ps_config._report_time = the_time
        ps_config._report_traceback = traceback_message
        ps_config._report_exception = exc_message
    try:
...
def get_test_id(test):
    ...
    try:
        ...
        if test._ps_test_identifier and len(str(test._ps_test_identifier)) > 6:
            test_id = test._ps_test_identifier
    return test_id
...
def get_test_name(test):
    ...
    if test._ps_test_identifier and len(str(test._ps_test_identifier)) > 6:
        test_name = test._ps_test_identifier
    return test_name

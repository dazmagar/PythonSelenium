...
from pythonselenium.core.style_sheet import get_report_style
...
def save_test_failure_data(test, name, folder=None):
    ...
    data_to_save = []
    if not hasattr(ps_config, "_report_test_id"):
        exc_message = "(Unknown Exception)"
        traceback_message = ""
        if hasattr(ps_config, "_report_traceback"):
            traceback_message = str(ps_config._report_traceback)
        if hasattr(ps_config, "_report_exception"):
            if type(ps_config._report_exception) is tuple:
                exc_message = str(ps_config._report_exception[1].message)
            else:
                exc_message = str(ps_config._report_exception)
        ...
        return
    data_to_save.append(ps_config._report_test_id)
    data_to_save.append(
        "--------------------------------------------------------------------"
    )
    data_to_save.append("Last Page: %s" % ps_config._report_fail_page)
    data_to_save.append(" Duration: %s" % ps_config._report_duration)
    data_to_save.append("  Browser: %s" % ps_config._report_browser)
    data_to_save.append("   Driver: %s" % ps_config._report_driver)
    data_to_save.append("Timestamp: %s" % ps_config._report_timestamp)
    data_to_save.append("     Date: %s" % ps_config._report_date)
    data_to_save.append("     Time: %s" % ps_config._report_time)
    data_to_save.append(
        "--------------------------------------------------------------------"
    )
    data_to_save.append("Traceback: %s" % ps_config._report_traceback)
    data_to_save.append("Exception: %s" % ps_config._report_exception)
    failure_data_file.writelines("\r\n".join(data_to_save))
    failure_data_file.close()
    ...
    for line in page_results_list:
        ...
        if line[1] == '"FAILED!"':
            ...
            if len(display_url) < 7:
                display_url = ps_config._report_fail_page
                actual_url = ps_config._report_fail_page

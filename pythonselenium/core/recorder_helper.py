"""Generating PythonSelenium code from the Recorder"""


def generate_pysel_code(srt_actions):
    ps_actions = []
        ...
        if action[0] == "begin" or action[0] == "_url_":
            if "%" in action[2]:
                try:
                    ...
                    pass
            if '"' not in action[2]:
                ps_actions.append('self.open("%s")' % action[2])
            ...
            if "%" in action[2]:
                try:
                    ...
                    pass
            if '"' not in action[2]:
                ps_actions.append('self.open_if_not_url("%s")' % action[2])
            elif "'" not in action[2]:
                ps_actions.append("self.open_if_not_url('%s')" % action[2])
            else:
                ps_actions.append(
            ...
            method = "click"
            if '"' not in action[1]:
                ps_actions.append('self.%s("%s")' % (method, action[1]))
            else:
                ps_actions.append("self.%s('%s')" % (method, action[1]))
        elif action[0] == "dbclk":
            method = "double_click"
            ...
            ps_actions.append("self.switch_to_default_content()")
        elif action[0] == "sw_pf":
            ps_actions.append("self.switch_to_parent_frame()")
        elif action[0] == "s_c_f":
            ...
            if nested:
                ...
                ps_actions.append("self.%s()" % method)
            else:
                ps_actions.append("self.%s()" % method)
        elif action[0] == "sleep":
            method = "sleep"
            ...
            ps_actions.append("self.accept_alert()")
        elif action[0] == "dis_a":
            ps_actions.append("self.dismiss_alert()")
        elif action[0] == "hi_li":
            method = "highlight"
            if '"' not in action[1]:
                ps_actions.append('self.%s("%s")' % (method, action[1]))
            ...
            dest = action[1][1]
            if not dest:
                ps_actions.append('self.%s("%s")' % (method, file_url))
            else:
                ps_actions.append(
            ...
            ps_actions.append("self.process_deferred_asserts()")
        elif action[0] == "a_d_m":
            ps_actions.append("self.activate_demo_mode()")
        elif action[0] == "d_d_m":
            ps_actions.append("self.deactivate_demo_mode()")
        elif action[0] == "c_l_s":
            ps_actions.append("self.clear_local_storage()")
        elif action[0] == "c_s_s":
            ps_actions.append("self.clear_session_storage()")
        elif action[0] == "d_a_c":
            ps_actions.append("self.delete_all_cookies()")
        elif action[0] == "go_bk":
            ps_actions.append("self.go_back()")
        elif action[0] == "go_fw":
            ps_actions.append("self.go_forward()")
        elif action[0] == "c_box":

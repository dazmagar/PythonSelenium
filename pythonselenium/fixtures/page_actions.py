...
from pythonselenium.fixtures import constants, page_utils, shared_utils
...
def wait_for_element_visible(
    ...
    if not element and by != "link text":
        ...
        if not is_present:
            ...
            timeout_exception(NoSuchElementException, msg or message)
        ...
        timeout_exception(ElementNotVisibleException, msg or message)
        ...
        timeout_exception(LinkTextNotFoundException, msg or message)
    else:
        return element
...
def wait_for_text_visible(
    ...
    for x in range(int(timeout * 10)):
        ...
        try:
            element = driver.find_element(by=by, value=selector)
            is_present = True
            if element.tag_name.lower() in ["input", "textarea"] and not shared_utils.is_safari(driver):
...
def wait_for_exact_text_visible(
    ...
    if not element:
        ...
        timeout_exception(TextNotVisibleException, msg or message)
    else:
        return element
...
def wait_for_attribute(
    ...
    if not element:
        ...
        timeout_exception(NoSuchAttributeException, msg or message)
    else:
        return element
...
def wait_for_element_clickable(
    ...
    if not element and by != "link text":
        ...
        timeout_exception(ElementNotInteractableException, msg or message)
...
def wait_for_element_absent(
    ...
    timeout_exception(Exception, msg or message)
...
def wait_for_exact_text_not_visible(
    ...
    if timeout == 1:
        plural = ""
    message = "Exact text {%s} for {%s} was still visible after %s second%s!" % (
        text,
        selector,
        timeout,
        plural,
...
def __switch_to_window(driver, window_handle, uc_lock=True):
    if hasattr(driver, "_is_using_uc") and driver._is_using_uc and uc_lock:

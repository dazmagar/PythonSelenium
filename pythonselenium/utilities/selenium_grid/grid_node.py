...
def invalid_run_command():
    exp = "  ** grid-node **\n\n"
    exp += "  Usage:\n"
    exp += "        pythonselenium grid-node {start|stop|restart} [OPTIONS]\n"
    exp += "  Options:\n"
    ...
    exp += "  Example:\n"
    exp += "        pythonselenium grid-node start --hub=127.0.0.1\n"
    exp += "  Output:\n"
...
def main():
    ...
    if not is_chromedriver_on_path():
        ...
        ps_install.main(override="chromedriver")
        sys.argv = sys_args  # Put back the original sys args

    dir_path = os.path.dirname(os.path.realpath(__file__))
    num_args = len(sys.argv)
    if sys.argv[0].split("/")[-1] == "pythonselenium" or (sys.argv[0].split("\\")[-1] == "pythonselenium") or (sys.argv[0].split("/")[-1] == "pysel") or (sys.argv[0].split("\\")[-1] == "pysel"):
        if num_args < 3:
            invalid_run_command()
    else:
        invalid_run_command()
    ...
    if shared_utils.is_linux() or shared_utils.is_mac():
        ...
        if grid_hub_command == "start" or grid_hub_command == "restart":
            shell_command = (
                ...
                """maxInstances=5,version=latest,seleniumProtocol=WebDriver""" % (dir_path, server_ip)
            )

...
def invalid_run_command(msg=None):
    exp = "  ** grid-hub **\n\n"
    exp += "  Usage:\n"
    exp += "        pythonselenium grid-hub {start|stop|restart} [OPTIONS]\n"
    exp += "  Options:\n"
    ...
    exp += "  Example:\n"
    exp += "        pythonselenium grid-hub start\n"
    exp += "  Output:\n"
...
def main():
    ...
    if shared_utils.is_linux() or shared_utils.is_mac():
        ...
        if grid_hub_command == "start" or grid_hub_command == "restart":
            shell_command = """java -jar %s/selenium-server-standalone.jar -role hub """ """-timeout %s -browserTimeout 170 -port 4444""" "" % (dir_path, timeout)

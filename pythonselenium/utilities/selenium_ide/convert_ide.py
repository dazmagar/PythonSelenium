...
Python WebDriver unittest file into PythonSelenium Python file.
...
Usage:
        pythonselenium convert [PYTHON_WEBDRIVER_UNITTEST_FILE].py
Output:
        [NEW_FILE_PS].py  (adds "_PS" to the original file name)
...
from pythonselenium.fixtures import js_utils


def main():
    expected_arg = "[A PYTHON_WEBDRIVER_UNITTEST_FILE exported from a Katalon/Selenium-IDE recording].py"
    num_args = len(sys.argv)
    if sys.argv[0].split("/")[-1] == "pythonselenium" or (sys.argv[0].split("\\")[-1] == "pythonselenium"):
        if num_args < 3 or num_args > 3:
            raise Exception("\n\n* INVALID RUN COMMAND! *  Usage:\n'pythonselenium convert %s'\n" % expected_arg)
    elif sys.argv[0].split("/")[-1] == "pysel" or (sys.argv[0].split("\\")[-1] == "pysel"):
        if num_args < 3 or num_args > 3:
            raise Exception("\n\n* INVALID RUN COMMAND! *  Usage:\n'pysel convert %s'\n" % expected_arg)
    else:
    ...
    pythonselenium_lines = []
    pythonselenium_lines.append("from pythonselenium import BaseCase")
    pythonselenium_lines.append("BaseCase.main(__name__, __file__)")
    pythonselenium_lines.append("")  # flake8 needs the whitespace
    pythonselenium_lines.append("")
    ...
    for line in code_lines:
        ...
        # Keep PythonSelenium classes if already used in the test script
        data = re.findall(r"^class\s\S+\(BaseCase\):\s*$", line)
        if data:
            pythonselenium_lines.append(line)
            continue
        ...
        if data:
            ...
            pythonselenium_lines.append(data)
            continue
        ...
        if data:
            ...
            if method_name.startswith("test_"):
                in_test_method = True
                pythonselenium_lines.append(data.group())
            else:
                in_test_method = False
            continue
        ...
        if data:
            ...
            pythonselenium_lines.append(command)
            continue
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_id\(\"(\S+)\"\)""" r"""\.submit\(\)\s*$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_name\(\"(\S+)\"\)""" r"""\.click\(\)\s*$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_name\(\"(\S+)\"\)""" r"""\.submit\(\)\s*$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_css_selector\(\"([\S\s]+)\"\)""" r"""\.click\(\)\s*$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_css_selector\(\"([\S\s]+)\"\)""" r"""\.submit\(\)\s*$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_css_selector\(\"([\S\s]+)\"\)""" r"""\.send_keys\(\"([\S\s]+)\"\)\s*$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_css_selector\(\"([\S\s]+)\"\)""" r"""\.send_keys\(Keys\.([\S]+)\)\s*$""",
            line,
        )
        if data:
            uses_keys = True
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_xpath\(\"([\S\s]+)\"\)""" r"""\.send_keys\(\"([\S\s]+)\"\)\s*$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_xpath\(\"([\S\s]+)\"\)""" r"""\.send_keys\(Keys\.([\S]+)\)\s*$""",
            line,
        )
        if data:
            uses_keys = True
        ...
        data = re.match(
            r"""^(\s*)Select\(driver\.find_element_by_css_selector\(""" r"""\"([\S\s]+)\"\)\)\.select_by_visible_text\(""" r"""\"([\S\s]+)\"\)\s*$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)Select\(driver\.find_element_by_id\(""" r"""\"([\S\s]+)\"\)\)\.select_by_visible_text\(""" r"""\"([\S\s]+)\"\)\s*$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)Select\(driver\.find_element_by_xpath\(""" r"""\"([\S\s]+)\"\)\)\.select_by_visible_text\(""" r"""\"([\S\s]+)\"\)\s*$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)Select\(driver\.find_element_by_name\(""" r"""\"([\S\s]+)\"\)\)\.select_by_visible_text\(""" r"""\"([\S\s]+)\"\)\s*$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_xpath\(u?\"([\S\s]+)\"\)""" r"""\.click\(\)\s*$""",
            line,
        )
        if data:
            whitespace = data.group(1)
            ...
            if ".//*[normalize-space(text())" in xpath and ("normalize-space(.)='" in xpath):
                x_match = re.match(
                    r"""^[\S\s]+normalize-""" r"""space\(\.\)=\'([\S\s]+)\'\]\)[\S\s]+""",
                    xpath,
                )
                if x_match:
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_xpath\(u?\"([\S\s]+)\"\)""" r"""\.submit\(\)\s*$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_link_text\(u?\"([\S\s]+)\"\)""" r"""\.click\(\)\s*$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)([\S\s]*)self\.is_element_present\(By.ID, """ r"""u?\"([\S\s]+)\"\)([\S\s]*)$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)([\S\s]*)self\.is_element_present\(By.CSS_SELECTOR, """ r"""u?\"([\S\s]+)\"\)([\S\s]*)$""",
            line,
        )
        if data:
            whitespace = data.group(1)
    ...
    for line in lines:
        ...
        if in_inefficient_wait:
            ...
            data = re.match(
                r"""^\s*if self.is_link_text_present""" r"""\("([\S\s]+)"\): break\s*$""",
                line,
            )
            if data:
                uni = ""
    ...
    for line_num in range(len(lines)):
        data = re.match(
            r"""^\s*self.wait_for_element""" r"""\((["|'])([\S\s]+)(["|'])\)""" r"""\s*$""",
            lines[line_num],
        )
        if data:
            ...
            if int(line_num) < num_lines - 1:
                ...
                if data2:
                    continue
                regex_string = r"""^\s*self.type\(["|']""" "" + selector + "" "" + r"""["|'], [\S\s]+\)\s*$"""
                data2 = re.match(regex_string, lines[line_num + 1])
                if data2:
                    continue
        pythonselenium_lines.append(lines[line_num])
    ...
    pythonselenium_code = ""
    if uses_keys:
        pythonselenium_code += "from selenium.webdriver.common.keys import Keys\n"
    if uses_select:
        pythonselenium_code += "from selenium.webdriver.support.ui import Select\n"
    for line in pythonselenium_lines:
        pythonselenium_code += line
        pythonselenium_code += "\n"
    # print(pythonselenium_code)  # (For debugging)

    # Create PythonSelenium test file
    base_file_name = webdriver_python_file.split(".py")[0]
    converted_file_name = base_file_name + "_PS.py"
    out_file = codecs.open(converted_file_name, "w+", encoding="utf-8")
    out_file.writelines(pythonselenium_code)
    out_file.close()

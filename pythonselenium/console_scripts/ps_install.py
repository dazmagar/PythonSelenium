"""
Downloads the specified webdriver to "pythonselenium/drivers/"

Usage:
         pysel get {chromedriver|geckodriver|edgedriver|
...
def invalid_run_command():
    exp = "  ** get / install **\n\n"
    exp += "  Usage:\n"
    exp += "           pythonselenium install [DRIVER] [OPTIONS]\n"
    exp += "           OR       pysel install [DRIVER] [OPTIONS]\n"
    exp += "           OR  pythonselenium get [DRIVER] [OPTIONS]\n"
    exp += "           OR           pysel get [DRIVER] [OPTIONS]\n"
    ...
    exp += "  Examples:\n"
    exp += "           pysel get chromedriver\n"
    exp += "           pysel get geckodriver\n"
    exp += "           pysel get edgedriver\n"
    exp += "           pysel get chromedriver 114\n"
    exp += "           pysel get chromedriver 114.0.5735.90\n"
    exp += "           pysel get chromedriver stable\n"
    exp += "           pysel get chromedriver beta\n"
    exp += "           pysel get chromedriver -p\n"
    exp += "  Output:\n"
    exp += "          Downloads the webdriver to pythonselenium/drivers/\n"
...
def get_cft_known_good_versions():
    if hasattr(ps_config, "cft_kgv_json") and ps_config.cft_kgv_json:
        return ps_config.cft_kgv_json
    cft_ngv_url = (
        "https://googlechromelabs.github.io/"
        "chrome-for-testing/known-good-versions.json"
    )
    ps_config.cft_kgv_json = requests_get(cft_ngv_url)
    return ps_config.cft_kgv_json


def get_cft_latest_versions_per_milestone():
    if hasattr(ps_config, "cft_lvpm_json") and ps_config.cft_lvpm_json:
        return ps_config.cft_lvpm_json
    cft_lvpm_url = (
        "https://googlechromelabs.github.io/"
        "chrome-for-testing/latest-versions-per-milestone.json"
    )
    ps_config.cft_lvpm_json = requests_get(cft_lvpm_url)
    return ps_config.cft_lvpm_json
...
def get_latest_chromedriver_version(channel="Stable"):
    try:
        if hasattr(ps_config, "cft_lkgv_json") and ps_config.cft_lkgv_json:
            return ps_config.cft_lkgv_json["channels"][channel]["version"]
        req = requests_get(
            "https://googlechromelabs.github.io/"
            "chrome-for-testing/last-known-good-versions.json"
        )
        if req and req.ok:
            ps_config.cft_lkgv_json = req.json()
...
def log_d(message):
    ...
    if (
        hasattr(ps_config.settings, "HIDE_DRIVER_DOWNLOADS")
        and ps_config.settings.HIDE_DRIVER_DOWNLOADS
    ):
...
def main(override=None, intel_for_uc=None, force_uc=None):
    if override:
        found_proxy = None
        if hasattr(ps_config, "proxy_driver") and ps_config.proxy_driver:
            if " --proxy=" in " ".join(sys.argv):
                for arg in sys.argv:
                    if arg.startswith("--proxy="):
                        found_proxy = arg
        ...
        if override == "chromedriver":
            sys.argv = ["pythonselenium", "get", "chromedriver"]
        elif override.startswith("chromedriver "):
            extra = override.split("chromedriver ")[1]
            sys.argv = ["pythonselenium", "get", "chromedriver", extra]
        elif override == "edgedriver":
            sys.argv = ["pythonselenium", "get", "edgedriver"]
        elif override.startswith("edgedriver "):
            extra = override.split("edgedriver ")[1]
            sys.argv = ["pythonselenium", "get", "edgedriver", extra]
        elif override == "geckodriver":
            sys.argv = ["pythonselenium", "get", "geckodriver"]
        elif override.startswith("geckodriver "):
            extra = override.split("geckodriver ")[1]
            sys.argv = ["pythonselenium", "get", "geckodriver", extra]
        elif override == "iedriver":
            sys.argv = ["pythonselenium", "get", "iedriver"]
        elif override.startswith("iedriver "):
            extra = override.split("iedriver ")[1]
            sys.argv = ["pythonselenium", "get", "iedriver", extra]
        if found_proxy:
            sys.argv.append(found_proxy)

    num_args = len(sys.argv)
    if (
        "pysel" in sys.argv[0].lower()
        or ("pythonselenium" in sys.argv[0].lower())
    ):
    ...
    if name == "chromedriver" or name == "uc_driver":
        ...
        if (
            ...
            try:
                from pythonselenium.core import detect_b_ver

...
Usage:
    pythonselenium recorder [OPTIONS]
...
from pythonselenium.fixtures import page_utils, shared_utils

ps_config.rec_subprocess_p = None
ps_config.rec_subprocess_used = False
...
def show_already_recording_warning():
    messagebox.showwarning(
        "PythonSelenium Recorder: Already Running!",
...
def do_recording(file_name, url, overwrite_enabled, use_chrome, window):
    poll = None
    if ps_config.rec_subprocess_used:
        poll = ps_config.rec_subprocess_p.poll()
    if not ps_config.rec_subprocess_used or poll is not None:
        pass
    else:
    ...
    if not page_utils.is_valid_url(url):
        if page_utils.is_valid_url("https://" + url):
            url = "https://" + url
    if not page_utils.is_valid_url(url):
        messagebox.showwarning("Invalid URL", "Enter a valid URL! (Eg. wikipedia.org)")
    else:
        ...
        if "--rec-behave" in command_args or "--behave" in command_args or "--gherkin" in command_args:
            add_on = " --rec-behave"
        command = "%s -m pythonselenium mkrec %s --url=%s --gui" % (sys_executable, file_name, url)
        if '"' not in url:
            command = '%s -m pythonselenium mkrec %s --url="%s" --gui' % (sys_executable, file_name, url)
        elif "'" not in url:
            command = "%s -m pythonselenium mkrec %s --url='%s' --gui" % (sys_executable, file_name, url)
        ...
        if ps_config.rec_subprocess_used:
            poll = ps_config.rec_subprocess_p.poll()
        if not ps_config.rec_subprocess_used or poll is not None:
            ps_config.rec_subprocess_p = subprocess.Popen(command, shell=True)
            ps_config.rec_subprocess_used = True
        else:
            show_already_recording_warning()
        send_window_to_front(window)
...
def create_tkinter_gui():
    ...
    window = tk.Tk()
    window.title("PythonSelenium Recorder App")
    ...
    entry.bind(
        "<Return>",
        (lambda _: do_recording(fname.get(), url.get(), cbx.get(), cbb.get(), window)),
    )
    tk.Button(
        window,
        text="Record",
        fg="red",
        command=lambda: do_recording(fname.get(), url.get(), cbx.get(), cbb.get(), window),
    ).pack()
    tk.Label(window, text="").pack()
    ...
    try:
        tk.Button(
            ...
            fg="teal",
            command=lambda: do_playback(fname.get(), cbb.get(), window, demo_mode=True),
        ).pack()
    except Exception:
        tk.Button(
            window,
...
def show_still_running_warning():
    ...
    messagebox.showwarning(
        "PythonSelenium Recorder: Still Running!",
...
if __name__ == "__main__":
    print('To open the Recorder Desktop App: "PythonSelenium recorder"')

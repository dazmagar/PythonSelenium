"""
Converts a PythonSelenium Python file into one that uses the Page Object Pattern.

Usage:
        pythonselenium objectify [PYTHONSELENIUM_FILE].py
Output:
...
def invalid_objectify_run_command():
    exp = "  ** objectify **\n\n"
    exp += "  Usage:\n"
    exp += "         pythonselenium objectify [PYTHONSELENIUM_FILE]\n"
    exp += "  Options:\n"
    exp += "         -c, --comments  (Add object selectors to the comments.)\n"
    exp += "                         (Default: No added comments.)\n"
    exp += "  Output:\n"
    exp += "         Converts a PythonSelenium file into one that uses\n"
    ...
    exp += '         (pythonselenium "objectify" has the same outcome as\n'
...
def invalid_inject_objects_run_command():
    exp = "  ** inject-objects **\n\n"
    exp += "  Usage:\n"
    exp += "         pythonselenium inject-objects [PYTHONSELENIUM_FILE]\n"
    exp += "  Options:\n"
    exp += "         -c, --comments  (Add object selectors to the comments.)\n"
    exp += "                         (Default: No added comments.)\n"
    exp += "  Output:\n"
    ...
    exp += "         the selected PythonSelenium file.\n"
    raise Exception("INVALID RUN COMMAND!\n\n%s" % exp)
...
def invalid_extract_objects_run_command():
    exp = "  ** extract-objects **\n\n"
    exp += "  Usage:\n"
    exp += "         pythonselenium extract-objects [PYTHONSELENIUM_FILE]\n"
    exp += "  Output:\n"
    exp += "         Creates page objects based on selectors found in a\n"
    exp += "         PythonSelenium file and saves those objects to the\n"
...
def invalid_revert_objects_run_command():
    exp = "  ** revert-objects **\n\n"
    exp += "  Usage:\n"
    exp += "         pythonselenium revert-objects [pythonselenium_PYTHON_FILE]\n"
    exp += "  Options:\n"
    exp += "         -c, --comments  (Keep existing comments for the lines.)\n"
    exp += "                         (Default: No comments are kept.)\n"
    exp += "  Output:\n"
    exp += '         Reverts the changes made by "pythonselenium objectify" or\n'
    exp += '         "pythonselenium inject-objects" when run against a\n'
    exp += "         PythonSelenium file. Objects will get replaced by\n"
...
def process_test_file(
    ...
    for line in code_lines:
        ...
        # pythonselenium_lines.append("### " + line)  # untouched lines (Debug)
        pythonselenium_lines.append(line)
        continue

    return pythonselenium_lines, page_selectors, changed
...
def main(shell_command):
    expected_arg = "[A PythonSelenium file]"
    ...
    if (
        sys.argv[0].split("/")[-1] == "pythonselenium"
        or (sys.argv[0].split("\\")[-1] == "pythonselenium")
        or (sys.argv[0].split("/")[-1] == "pysel")
        or (sys.argv[0].split("\\")[-1] == "pysel")
    ):
        if num_args < 3:
            invalid_run_command(shell_command)
    ...
    pythonselenium_file = command_args[0]
    if not pythonselenium_file.endswith(".py"):
        raise Exception(
            "\n\n`%s` is not a Python file!\n\n"
            "Expecting: %s\n" % (pythonselenium_file, expected_arg)
        )

    with open(pythonselenium_file, "r", encoding="utf-8") as f:
        all_code = f.read()
    if "def test_" not in all_code:
        raise Exception(
            "\n\n`%s` is not a valid PythonSelenium unittest file!\n"
            "\nExpecting: %s\n" % (pythonselenium_file, expected_arg)
        )
    code_lines = all_code.split("\n")
    pythonselenium_lines, page_selectors, changed = process_test_file(code_lines)
    ...
    if shell_command == "inject-objects" or shell_command == "objectify":
        pythonselenium_lines, page_selectors, changed = process_test_file(
            ...
            for line in pythonselenium_lines:
                if line.startswith("from") and "import" in line and not fit_in:
                    ...
                    for add_me in added_classes:
                        ...
                        ps_lines.append(import_line)
                ps_lines.append(line)
            pythonselenium_lines = ps_lines
    ...
    for line in pythonselenium_lines:
        ...
        pythonselenium_code += "\n"
    pythonselenium_code = pythonselenium_code[:-1]
    # print (pythonselenium_code)  # (For debugging)

    # Create pythonselenium test file
    base_file_name = pythonselenium_file.split(".py")[0]

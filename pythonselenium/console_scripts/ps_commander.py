"""
Launches PythonSelenium Commander | GUI for pytest.

Usage:
      pythonselenium commander [OPTIONAL PATH or TEST FILE]
               pysel commander [OPTIONAL PATH or TEST FILE]
            pythonselenium gui [OPTIONAL PATH or TEST FILE]
...
def create_tkinter_gui(tests, command_string, files, solo_tests):
    root = tk.Tk()
    root.title("PythonSelenium Commander | GUI for pytest")
...
def main():
    ...
    proc = subprocess.Popen(
        '"%s" -m pytest --collect-only -q --rootdir="./" %s' % (sys.executable, command_string),
    ...
    for row in output.replace("\r", "").split("\n"):
        if ("::") in row:
            tests.append(row)
    if not tests:
        error_msg = "No tests found! Exiting PythonSelenium Commander..."
...
if __name__ == "__main__":
    print('To open pysel Commander, type "pysel commander" or "pysel gui"')

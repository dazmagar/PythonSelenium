"""
Launches PythonSelenium Behave Commander | GUI for Behave.

Usage:
      pythonselenium behave-gui [OPTIONAL PATH or TEST FILE]
               pysel behave-gui [OPTIONAL PATH or TEST FILE]

Examples:
      pysel behave-gui
      pysel behave-gui features/
      pysel behave-gui features/calculator.feature
...
def create_tkinter_gui(tests, command_string, t_count, f_count, s_tests):
    root = tk.Tk()
    root.title("PythonSelenium Behave Commander | GUI for Behave")
...
def main():
    ...
    proc = subprocess.Popen(
        '"%s" -m behave -d %s --show-source' % (sys.executable, command_string),
    ...
    if not tests:
        ...
        err_msg_3 = "Exiting pysel Behave Commander..."
...
if __name__ == "__main__":
    print('To open pysel Behave Commander, type "pysel behave-gui"')

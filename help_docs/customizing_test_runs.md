...
## pytest options for PythonSelenium

🎛️ PythonSelenium's [pytest plugin](/pythonselenium/plugins/pytest_plugin.py) lets you customize test runs from the CLI (Command-Line Interface), which adds options for setting/enabling the browser type, Dashboard Mode, Demo Mode, Headless Mode, Mobile Mode, Multi-threading Mode, Recorder Mode, reuse-session mode, proxy config, user agent config, browser extensions, html-report mode, and more.

🎛️ Here are some examples of configuring tests, which can be run from the [examples/](/examples) folder:
...
# Run tests while changing PythonSelenium default settings
...
🎛️ If you're using ``pytest`` for running tests outside of the PythonSelenium repo, you'll want a copy of [pytest.ini](/pytest.ini) at the base of the new folder structure. If using ``nosetests``, the same applies for [setup.cfg](/setup.cfg).
...
🎛️ PythonSelenium provides additional ``pytest`` command-line options for tests:
...
(For more details, see the full list of command-line options **[here](/pythonselenium/plugins/pytest_plugin.py)**.)

🎛️ You can also view a list of popular ``pytest`` options for PythonSelenium by typing:

```bash
pythonselenium options
```
...
<h3>Example tests using Logging:</h3>
...
🔵 You can override the default wait time by either updating [settings.py](/pythonselenium/config/settings.py) or by using ``--demo-sleep=NUM`` when using Demo Mode. (NOTE: If you use ``--demo-sleep=NUM`` without using ``--demo``, nothing will happen.)

```bash
pytest my_first_test.py --demo --demo-sleep=1.2
```

<h3>Passing additional data to tests:</h3>

If you want to pass additional data from the command line to your tests, you can use ``--data=STRING``. Now inside your tests, you can use ``self.data`` to access that.

<h3>Running tests multithreaded:</h3>
...
Visit [🗾 Locales](/help_docs/locale_codes.md) for a full list of codes.

--------

<h3>Changing the default driver version:</h3>

🔵 By default, PythonSelenium will make sure that the major driver version matches the major browser version for Chromium tests. (Eg. If Chrome `117.X` is installed and you have chromedriver `117.X`, then nothing happens, but if you had chromedriver `116.X` instead, then PythonSelenium would download chromedriver `117.X` to match the browser version.)
...
<h3>Customizing default settings:</h3>

🎛️ An easy way to override [pythonselenium/config/settings.py](/pythonselenium/config/settings.py) is by using a custom settings file.
Here's the command-line option to add to tests: (See [examples/custom_settings.py](/examples/custom_settings.py))
...
<h3>Running tests on a remote Selenium Grid:</h3>

🌐 PythonSelenium lets you run tests on remote Selenium Grids such as [BrowserStack](https://www.browserstack.com/automate#)'s Selenium Grid, [Sauce Labs](https://saucelabs.com/products/platform-configurator)'s Selenium Grid, other Grids, and even your own Grid:

🌐 For setting browser desired capabilities while running Selenium remotely, see the ReadMe located here: https://github.com/pythonselenium/PythonSelenium/tree/master/examples/capabilities
...
🌐 Or you can create your own Selenium Grid for test distribution. ([See this ReadMe for details](/pythonselenium/utilities/selenium_grid/ReadMe.md))
...
🌐 PythonSelenium also supports SOCKS4 and SOCKS5 proxies:
...
To make things easier, you can add your frequently-used proxies to PROXY_LIST in [proxy_list.py](/pythonselenium/config/proxy_list.py), and then use ``--proxy=KEY_FROM_PROXY_LIST`` to use the IP_ADDRESS:PORT of that key.
